...
from dataset import InvPatchBimacsTrainDataset, InvPatchBimacsTestDataset
from dataset import InvPatchBimacsTrainDataset_32_Frames, InvPatchBimacsTestDataset_32_Frames
from custom_tokenizer import ActionTokenizerBimacs
...
if __name__ == "__main__":
    ...
    RUNS_SAVE_PATH = f'{PROJECT_ROOT}/idm/idm_bimacs/artifacts/runs'
    exp_configurations = [
        *some_configuration
    ]
    ...
    for configuration in exp_configurations:
        ...
        TRAIN_SET = f"{PROJECT_ROOT}/idm/idm_bimacs/artifacts/dataset_splits/{SPLIT_FOLDER}/train_{SPLIT_FOLD}.json"
        TEST_SET = f"{PROJECT_ROOT}/idm/idm_bimacs/artifacts/dataset_splits/{SPLIT_FOLDER}/test_{SPLIT_FOLD}.json"
        _tokenizer = ActionTokenizerBimacs(context_len=MAX_CONTEXT_LEN)
        CONFIG_PATH = f"{PROJECT_ROOT}/config/decoder_config.pkl"
        IGNORE_INDEX = 0
        ...
        if NUM_FRAMES != 32:
            dataset_train = InvPatchBimacsTrainDataset(splits_path=TRAIN_SET, tokenizer=_tokenizer, actions_file=actions_file_path)
        elif NUM_FRAMES == 32:
            dataset_train = InvPatchBimacsTrainDataset_32_Frames(splits_path=TRAIN_SET, tokenizer=_tokenizer, actions_file=actions_file_path)
        ...
        torch.save(trained_model.state_dict(), f"{CHECKPOINTS_DIR}/BIMACS_action_gpt.pt")
        trained_model.eval()

        # Run evaluation on content-style model
        if NUM_FRAMES != 32:    
            dataset_test = InvPatchBimacsTestDataset(splits_path=TEST_SET, actions_file=actions_file_path)
        elif NUM_FRAMES == 32:
            dataset_test = InvPatchBimacsTestDataset_32_Frames(splits_path=TEST_SET, actions_file=actions_file_path)
